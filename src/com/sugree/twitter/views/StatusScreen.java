...
//#endif
		...
		retweetCommand = new Command("Retweet", Command.SCREEN, 3);
		addCommand(retweetCommand);
		favoriteCommand = new Command("Toggle Favorite", Command.SCREEN, 4);
		addCommand(favoriteCommand);
		directCommand = new Command("Direct Message", Command.SCREEN, 6);
		addCommand(directCommand);

//#if polish.midp2 || polish.midp3
		linkCommand = new Command("Link", Command.SCREEN, 5);
//#if project.HyperlinkList
...
//#if (polish.midp2 || polish.midp3) && project.HyperlinkList
	public void handleTwitPic(String url) {
		...
		if (slash >= 0) {
			...
			} else if ((host.equals("pic.gd") ||
					host.equals("tweetphoto.com") ||
					host.equals("api.plixi.com")) &&
					path.indexOf('/') == -1) {
				linkList.append("http://api.plixi.com/api/tpapi.svc/imagefromurl?size=thumbnail&url="+orig, null);
			}
		}
	}
//#endif
